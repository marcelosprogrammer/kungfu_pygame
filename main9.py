...
# cria o metodo para detectar a colisão
def colisao(meteoroX, meteoroY, lutadorX, lutadorY):
    distancia = math.sqrt(math.pow(meteoroX - lutadorX, 2) + (math.pow(meteoroY - lutadorY, 2)))
    if distancia < 27:
        return True
    else:
        return False
...
def showpontos(x, y):
    score = font.render("Pontos Jogo " + str(ponto_total), True, (0, 0, 0))
...
# GERANDO O MOVIMENTO ======================================================
    ...
    for i in range(quantidade):
        ...
        # verifica se golpeou
        golpeou = colisao(meteoroX[i], meteoroY[i], lutador1X, lutador1Y)
        if golpeou:
           golpeSom = mixer.Sound("soco.mp3")
           golpeSom.play()
           ponto_total += 1
